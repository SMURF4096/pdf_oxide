...
// This test verifies the fix on HEAD (release/v0.3.41).
...
fn test_qr_format_sentinel_no_collision() {
    ...
    let svg_1d = BarcodeGenerator::generate_1d_svg(
        BarcodeType::Code128, "ABC", &BarcodeOptions::default()
    ).unwrap();
    
    // QR SVG — 2D matrix, much larger rect count
    let svg_qr = BarcodeGenerator::generate_qr_svg(
        "ABC", &QrCodeOptions::default().size(256)
    ).unwrap();
    ...
    let qr_y_values: Vec<&str> = svg_qr.split("y=\"").skip(1)
        .map(|s| s.split('"').next().unwrap_or(""))
        .collect();
    let distinct_y: std::collections::HashSet<&str> = qr_y_values.iter().copied().collect();
    assert!(distinct_y.len() > 3, "QR must have multiple distinct y values (2D grid), got {:?}", distinct_y.len());
    
    let all_y_zero = svg_1d.split("y=\"").skip(1)
        .map(|s| s.split('"').next().unwrap_or(""))
        .all(|y| y == "0");
